...
`https://www.cinema-comoedia.com/programme-accessible/`  
...
| `programme_sample.html` | HTML local pour tests (optionnel) |
...
### 1. Inspecter la structure HTML (si le parseur échoue)

Le site utilise Gatsby (cinema-comoedia.com). Si la structure change :

```bash
# Avec fetch (nécessite accès réseau)
python3 inspect_html.py

# Ou avec un fichier HTML sauvegardé localement
python3 inspect_html.py --file programme_sample.html
```

Cela affiche les classes CSS et la structure pour adapter `parse_programme()`.
...
# Tester avec un fichier HTML local (évite le fetch)
python3 scraper.py --file programme_sample.html --dry-run --no-omdb --no-filter
```
...
Structure actuelle (Gatsby, mars 2026) :
- `<h2>Du X au Y mois Z</h2>` → période de la semaine
- `<h5>Titre</h5>` + bloc suivant avec "Réalisé par", "Film produit", "Séances prévues"
- Séances en texte inline : "mercredi, samedi 11h15, 13h35" ou "tous les jours à 20h50"

Si la structure change, adapter `parse_programme()` et `_extract_film_comoedia()`.
