...
- **Éventail — `.ev-fan` › `.ev-card`** : trois niveaux de profondeur (`data-niveau` 0/1/2, `data-cote` -1/0/1), décalages et rotation en variables (`--ev-dx1`, `--ev-dx2`, `--ev-rot`), opacité 70 % au niveau 2. **Un seul format : le portrait 2:3** — centre à `--gab-v`, côtés au gabarit réduit (× .9 puis × .81). Toute affiche y est recadrée en `object-fit:cover` / `object-position:top` (préserve le bloc-titre), **y compris une source paysage** : elle remplit un cadre portrait et perd ses côtés. **Don't** : réintroduire un gabarit horizontal — il abaissait la carte centrale et déséquilibrait l'éventail.
...
- **Apparition « tissu » — `.ev-tissu`** : à l'arrivée de la sélection, **chaque affiche** de l'éventail se déplie comme une étoffe qui se dépose (un canvas par carte, ≤ 5 contextes WebGL ; `profondeurZ` s'étage par `data-niveau`, les rangs du fond arrivant de plus loin) (shader WebGL2, `src/cloth.js`, réglé dans `design/lab/cloth.html`). Canvas superposé **dans la carte** — et non dans `.ev-fan` : il doit hériter de sa translation et de sa rotation, sinon le pli d'une carte latérale resterait au centre, à plat. `pointer-events:none`, `aria-hidden` : la carte dessous garde clic, focus et gabarit ; elle passe en `.tissu-en-cours` le temps du vol (image masquée, `overflow:visible` pour laisser sortir le pli). Une carte dont l'affiche n'est pas texturable retombe sur le fondu, au même tempo. Le `ratioCadre` est lu sur l'affiche **rendue**, donc il suit l'échelle élastique `--ev-u` et les rangs latéraux réduits sans les relire.
  **Déclenchement** : deux conditions cumulées — l'utilisateur a défilé au moins une fois (mémorisé pour la session), et l'éventail est dans la bande centrale de la fenêtre (`IntersectionObserver`, `rootMargin:-25%`). La condition de scroll n'est levée d'office que si la page **ne peut pas** défiler, sans quoi les cartes resteraient invisibles à jamais.
  **Une affiche exploitable au centre** : c'est la carte centrale qui porte le pli ; y placer une affiche que le shader ne peut pas texturer (cross-origin sans CORS) revient à désactiver l'effet. `evCentrerAfficheExploitable()` sonde les affiches et décale la sélection vers la plus proche qui convienne. Le test n'existant qu'une fois l'image chargée, la correction se fait à la volée (`evCardOrient`) — invisible, l'éventail étant encore escamoté. **Do** : la désarmer dès que l'utilisateur a navigué (`_evNavManuelle`), son choix prime. **Don't** : la laisser tourner hors de l'état `.ev-attente` — elle appelle `evFanApply`, qui fait TOURNER l'éventail : à chaque changement de mois, ça se voyait comme une flèche pressée toute seule. **Don't** : la laisser aboutir après le lancement de l'apparition — elle appelle `evFanApply`, qui détruit le tissu ; d'où `_evApparitionLancee` et l'attente de `_evChoixPromesse`.
  **Une seule fois, au mois d'arrivée** : l'apparition est un accueil. Elle ne se rejoue ni en changeant de mois, ni en revenant (`_evApparitionFaite`, `_evMoisAccueil`). **Do** : se caler sur le mois d'ARRIVÉE, pas sur la date du jour — `evMois` retombe sur le premier mois disponible quand le mois en cours est vide, et un test calendaire perdrait l'animation en silence.
  **Do** : précharger les mois adjacents en temps mort (`evPrechargerMoisVoisins`) — la même sonde sert au cache image et à l'arbitrage de la variante. **Do** : compter sur des affiches rapatriées (`scraper.py` → bucket `affiches`) — une origine sans en-tête CORS rend l'affiche intexturable et retire le pli à sa carte. **Do** : garder l'effet strictement facultatif — sans WebGL2, sans en-tête CORS sur l'affiche, ou sous `prefers-reduced-motion`, aucun canvas n'est créé et l'affiche s'affiche normalement. **Don't** : le déclencher sur la navigation de l'éventail — déplier est une **arrivée**, pas un déplacement ; `evFanApply` l'interrompt au contraire. **Don't** : laisser une carte en `.tissu-en-cours` sur un chemin d'erreur, elle resterait vide.
