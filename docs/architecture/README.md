...
| I8 | **Table `seances` gardée légère** (< plafond REST 1000) via cleanup J−10 | cleanup_old_seances.py ; front filtre `date >= today` (index.html). **Exception assumée (2026-08-12)** : l'écran d'arrivée (`src/intro.js`) pagine `seances` en entier sur 2 colonnes pour dater la *première* séance de chaque film — le plafond y est franchi par pagination explicite, pas subi ([Frontend](frontend.md)) | table qui gonfle → séances récentes tronquées à l'affichage ; côté intro, une pagination qui s'allonge → basculer sur une fonction SQL `nouveautes()` |
