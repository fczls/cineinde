...
| **Horizon de scraping des séances** | I8 (plafond REST 1000) | La dénormalisation de `evenement_seances` contourne le problème, elle ne le résout pas : la plupart des films du niveau 2 restent en « Séances non encore annoncées » jusqu'à ce que leur semaine arrive. Étendre Lumière sur N semaines (`?week=` accepte n'importe quelle date) pousse vers le plafond qui a motivé I8 — chantier séparé |
